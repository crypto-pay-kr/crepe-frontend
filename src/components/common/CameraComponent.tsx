  ...
  return (
    <div className="relative w-full h-full bg-black overflow-hidden rounded-xl shadow-lg">
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-center items-center space-x-4">
        ...
        <button
          onClick={flipCamera}
          className="relative p-1 rounded-full focus:outline-none transform transition-all bg-gray-800 bg-opacity-60 hover:bg-opacity-80 shadow-lg"
        >
      ...
      ${isSubjectDetected ? 'bg-white hover:bg-opacity-80 scale-100' : 'bg-gray-400 scale-95 cursor-not-allowed'}
    `}
        >
          <div className="w-10 h-10 rounded-full border-4 border-current flex items-center justify-center">
            <Camera size={16} className={isSubjectDetected ? 'text-black' : 'text-gray-600'} />
          </div>
        </button>
      </div>
